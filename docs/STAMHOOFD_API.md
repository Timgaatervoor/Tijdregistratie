...
De bestaande React/Vite-app op GitHub Pages gebruikt een aparte Cloudflare Worker als registratiebron. Deelnemers, optionele ticketgegevens, veldkeuzes en syncverslagen worden lokaal opgeslagen in dezelfde Dexie-database. Start, finish en schieten gebruiken deze Worker niet. CSV/Excel-import blijft beschikbaar.
...
6. Open **Deelnemers → Stamhoofd API**. Vul de Worker URL, het webshopdomein (bijvoorbeeld `shop.kidsatletiekdehaan.be`) en de aparte Worker-toegangscode in. Deze toegangscode blijft alleen in het geheugen tot het venster sluit; hij wordt niet opgeslagen. Vul hier nooit de Stamhoofd API-key in.
...
Er is tijdens ontwikkeling geen echte Stamhoofd API-key verstrekt. Daarom is geen vergelijking met die onbekende key mogelijk. Controleer bij deployment dat je uitsluitend `wrangler secret put` gebruikt. Frontend/build/history worden op secretpatronen gecontroleerd; een patrooncontrole is geen bewijs over een onbekende geheime waarde.
