...
import { searchShops, loadStamhoofd } from '../services/stamhoofdApi';
...
export function StamhoofdIntegrationModal({ participants, categories, onClose, onRefresh }: { participants: Participant[]; categories: Category[]; onClose: () => void; onRefresh: () => void }) {
  const [config, setConfig] = useState<StamhoofdConfig>({ id: '', workerUrl: '', domain: 'shop.kidsatletiekdehaan.be', fields: defaultFields, mapping: {}, productCategories: {} });
  ...
  useEffect(() => { void (async () => {
    try {
      ...
      setConfig(c => saved ?? { ...c, id: event.id });
  ...
  return <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-2 sm:p-6" role="dialog" aria-modal="true" aria-label="Stamhoofd integratie">
    <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-6xl max-h-[95vh] overflow-y-auto p-5 space-y-5 text-slate-200">
      ...
      <fieldset disabled={!!busy || !config.id} className="space-y-4 disabled:opacity-70">
        <legend className="font-bold mb-2">1. Verbinding</legend>
        <div className="grid sm:grid-cols-2 gap-3">
          <label>Worker URL<input className={input} type="url" placeholder="https://biathlon-stamhoofd.account.workers.dev" value={config.workerUrl} onChange={e => { update({ workerUrl: e.target.value, shop: undefined }); setSnapshot(undefined); setShops([]); }} /></label>
          ...
          <label>Worker-toegangscode<input className={input} type="password" autoComplete="off" value={token} onChange={e => setToken(e.target.value)} /><span className="text-xs text-slate-400">Aparte SYNC_ACCESS_TOKEN, alleen in geheugen. Vul hier nooit de Stamhoofd API-key in.</span></label>
        </div>
        <button className={button} onClick={() => run('Webshops zoeken', async () => { const result = await searchShops(config.workerUrl, config.domain, token); setShops(result.shops); setSelected(result.shops[0]?.id ?? ''); if (!result.shops.length) throw new Error('Geen webshops gevonden.'); })}>Zoek webshops</button>
