  ...
  return <section className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4 text-white">
    ...
    <fieldset disabled={busy} className="space-y-4">
      <div className="grid sm:grid-cols-3 gap-3">
        ...
        <label className="text-sm font-semibold text-slate-300">Interval (minuten)<input type="number" min="1" max="1440" value={settings.intervalMinutes || ''} onChange={e => update({ intervalMinutes: Number(e.target.value) })} className={`${ui.input} h-11 mt-1.5`} /></label>
        <label className="text-sm font-semibold text-slate-300">Standaardaantal deelnemers<input type="number" min="1" max="1000" value={settings.capacity || ''} onChange={e => update({ capacity: Number(e.target.value) })} className={`${ui.input} h-11 mt-1.5`} /></label>
      </div>
      ...
      <button type="button" className={`${ui.secondary} h-11`} onClick={() => run(async () => { validateWaveSettings(settings); const event = await db.events.toCollection().first(); if (!event) throw new Error('Maak eerst een evenement aan.'); await db.events.update(event.id, { waveSettings: settings }); setMessage('Wave-instellingen opgeslagen voor dit evenement.'); })}>Instellingen opslaan</button>
      <div className="border-t border-slate-700 pt-4 space-y-3">
        <h3 className="font-bold">Automatisch indelen op basis van inschrijving</h3>
        <label className="block text-sm font-semibold text-slate-300">Indeling<select value={grouping} onChange={e => { setGrouping(e.target.value as WaveGrouping); setPreview(undefined); }} className={`${ui.input} h-11 mt-1.5`}>
        ...
        <button type="button" className={`${ui.primary} h-11`} onClick={() => run(async () => { setPreview(undefined); const event = await db.events.toCollection().first(); if (!event) throw new Error('Maak eerst een evenement aan.'); setPreview(planWaves(event.id, waves, participants, settings, grouping)); })}>Indeling bekijken</button>
        {preview && <div className="space-y-3">
          ...
          <button type="button" disabled={!preview.assignments.length} className={`${ui.primary} h-11`} onClick={() => run(async () => { const count = await applyWavePlan(settings, grouping, preview); setPreview(undefined); setMessage(`${count} deelnemers in waves ingedeeld.`); onRefresh(); })}>Indeling toepassen</button>
        </div>}
      </div>
    </fieldset>
